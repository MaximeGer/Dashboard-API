...
public class WeatherController {
        ...
        @RequestMapping(value = "/weather/forecast/{cityname}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
        @ResponseBody
        public String getWeatherForecast(@PathVariable("cityname") String cityname) throws IOException, InterruptedException {
                HttpClient client = HttpClient.newHttpClient();
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create("pro.openweathermap.org/data/2.5/forecast/hourly?q=" + cityname + "&appid=525fed8a65a821cff57c8062da27fd8a"))
                        .build();
